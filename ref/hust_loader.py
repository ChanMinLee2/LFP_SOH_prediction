...
class HUSTPreprocessor(BasePreprocessor):
    def process(self, parentdir, **kwargs) -> List[BatteryData]:
        ...
        if not self.silent:
            cell_files = tqdm(cell_files, desc="Processing HUST cells")
        ...
        for cell_file in cell_files:
            ...
            for cycle in range(len(cell_data)):
                ...
                I = df["Current (mA)"].values / 1000.0  # noqa
                t = df["Time (s)"].values
            ...
            battery = BatteryData(
                ...
                charge_protocol=[
                    CyclingProtocol(rate_in_C=5.0, start_soc=0.0, end_soc=0.8),
                    CyclingProtocol(rate_in_C=1.0, start_soc=0.8, end_voltage_in_V=3.6),
                    CyclingProtocol(
                        voltage_in_V=3.6, start_voltage_in_V=3.6, end_soc=1.0
                    ),
                ],
                discharge_protocol=[
                    CyclingProtocol(
                        rate_in_C=float(rates[0]), start_soc=1.0, end_soc=0.6
                    ),
                    CyclingProtocol(
                        rate_in_C=float(rates[1]), start_soc=0.6, end_soc=0.4
                    ),
                    CyclingProtocol(
                        rate_in_C=float(rates[2]), start_soc=0.4, end_soc=0.2
                    ),
                    CyclingProtocol(rate_in_C=1.0, start_soc=0.2, end_voltage_in_V=2.0),
                ],
